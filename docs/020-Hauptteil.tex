...
es entstehen 3 cluster (0: 102 Personen, 1: 898 Personen, 2: 9 Personen)
\begin{table}[H]
    \centering
    ...
    \end{tabular}
\end{table}
2 fühlt sich von Arbeitgeber sehr wenig unterstützt, 0 besser als 1 unterstützt
...
\begin{table}[H]
    ...
    \end{tabular}
\end{table}
2 hatte früher schlechte Arbeitgeber, 0 durchschnittliche Erfahrung, 1 gute erfahrung, 
2 zeigt klares Muster von schlechter Umgebung (früher + aktuell)
...
\begin{table}[H]
    ...
    \end{tabular}
\end{table}
2 viel offener, 0 eher verschlossen, 1 wenig offen
...
\begin{table}[H]
    ...
    \end{tabular}
\end{table}
0 schlecht mental gesund, 2 guter zustand, 1 durchschnittlich

GESAMT: (aus diesen 5 features erkennen wir) 
CLUSTER 2: offene, mental stabile Personen trotz geringer Arbeitgeberunterstützung,
aber höhere stigma, also denken dass sie benachteilgt wären wenn sie mental krank wären
CLUSTER 0: verschlossen, schlechter mentaler Zustand, trotz besseren support
als Cluster 2
CLUSTER 1: dient eher als Vergleichsgruppe, alles durchschnittlich
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{pics/selected_features_radar_chart.png}
    \caption{Fünf selbst-generierte Features pro Cluster}
    \label{fig:radar_chart_features_per_cluster}
\end{figure}

Danach wurden andere Features untersucht, wie Job Roles, diese sind folgend verteilt:
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{pics/job_roles_per_cluster.png}
    \caption{Arbeitsstellen pro Cluster}
    \label{fig:job_roles_per_cluster}
\end{figure}
CLUSTER 0: viele aus community (writer, advocate), hr/admin und design,
und paar devops
CLUSTER 1: eher devops paar developer und Lead
CLUSTER 2: Developer und others

danach der Arbeitsland
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{pics/working_country_per_cluster.png}
    \caption{Arbeitsländer pro Cluster}
    \label{fig:working_country_per_cluster}
\end{figure}
CLUSTER 0: hauptsächlich Australia, Ireland aber auch Kanada und bisschen UK, USA
CLUSTER 1: Hauptsächlich Frankreich, Deutschland, Netherlands, Others, Schweden
UK, USA, bisschen auch Kanada
CLUSTER 2: hauptsächlich Brazil

danach wurde untersucht "Is your employer primarily a tech company/organization?\_1.0"
\begin{table}[H]
    \centering
    ...
    \end{tabular}
\end{table}
entscheidend ist, das bei 0 hauptsächlich die tech company ist, während bei 2 nicht

dann untersuchung auf Geschlecht
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{pics/gender_per_cluster.png}
    \caption{Geschlecht pro Cluster}
    \label{fig:gender_per_cluster}
\end{figure}
0 sind hauptsächlich Frauen und "andere", 2 sind hauptsächlich männer, 1 sind 
männer/frauen so 50/50

dann remote job?
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{pics/relative_remote_work.png}
    \caption{Remote Work pro Cluster (relativ)}
    \label{fig:remote_work_per_cluster}
\end{figure}
0 und 1 eher niemals remote, 2 viel öfter

dann ob bei potenziellen mentalen probleme die es mit ihren freunden/familien teilen würden
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{pics/sharing_mental_illness.png}
    \caption{Über MH-Problemen mit Familie teilen pro Cluster (relativ)}
    \label{fig:sharing_mh_issues_per_cluster}
\end{figure}
0 und 1 sind nicht offen über potenzielle mh den freunden und familie zu 
erzählen, bei 2 ist es eher ausgewogen

dann wie ihre mentale probleme die arbeit beeinflussen, wenn
- gut behandelt werden in arbeit
- NICHT gut behandelt werden in der arbeit
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{pics/problems_at_work_treated_effectively.png}
    \caption{Schwierigkeiten in der Arbeit bei guter Behandlung}
    \label{fig:problems_at_work_treated_effectively}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{pics/problems_at_work_not_treated_effectively.png}
    \caption{Schwierigkeiten in der Arbeit bei schlechter Behandlung}
    \label{fig:problems_at_work_not_treated_effectively}
\end{figure}
allgemein sieht man dass wenn leute nicht gut behandelt werden, führt es
öfter zu problemene und schwierigkeiten bei der arbeit
...
vielleicht noch bildlich darstellen oder so
